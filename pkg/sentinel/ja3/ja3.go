...
// Digest returns only the digest md5.
func SortedDigest(hello *tlsx.ClientHelloBasic) [md5.Size]byte {
	return md5.Sum(SortedBare(hello))
}

// DigestHex produce md5 hash from bare string.
func SortedDigestHex(hello *tlsx.ClientHelloBasic) string {
	return BareToDigestHex(SortedBare(hello))
}

func SortedBare(hello *tlsx.ClientHelloBasic) []byte {
	bare := Bare(hello)
	fields := bytes.Split(bare, []byte{sepFieldByte})

	// fields[0] = version (don't sort)
	// fields[1] = cipher suites
	// fields[2] = extensions
	// fields[3] = supported groups
	// fields[4] = supported points

	for i := 1; i < len(fields); i++ {
		values := bytes.Split(fields[i], []byte{sepValueByte})
		sort.Slice(values, func(a, b int) bool {
			na, _ := strconv.Atoi(string(values[a]))
			nb, _ := strconv.Atoi(string(values[b]))
			return na < nb
		})
		fields[i] = bytes.Join(values, []byte{sepValueByte})
	}

	return bytes.Join(fields, []byte{sepFieldByte})
}
